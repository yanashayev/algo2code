...
import java.util.Arrays;
    public class FloydWarshallBooleanCODE{
        ...
        public static void buildFWBooleanMatrix(boolean [][] bm){
            //if there is a k that can help bond two nodes by him page 3 FW
            // matrix calculation
            int n = bm.length;
            for (int k = 0; k<n; k++){ //k is the size of the fist arr
                for (int i = 0; i<n; i++){
                    for (int j = 0; j<n; j++){
        ...
        public static String [][] buildPathMatrix(boolean [][] bm){
            // the mat of path by strings!! page 3 back
            int n = bm.length;
        ...
        public static boolean isConnectedComplexN(boolean mat[][]){    //O(n) go trow the first line page 4 up
            boolean ans = true;
            for(int i=0; ans && i<mat.length; i++){
        ...
        //Problem 3: Number connected component how much
        //go to the mat and check the first line how much 1's there is and if we cross by 0 we go to the line
        //page 4 in my sicom- כמה רכיבי קשירות יש
        ...
        // by help array that go over the array and check if there is a 0 value page 4
        public static void connectComponentsOfGraphBoolean(boolean [][] mat){
            ...
            System.out.println("number of components = "+numComponentes); // how much components there is page 4
            for (int i=0; i<numComponentes; i++)
