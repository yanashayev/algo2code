package T4;
    ...
    public class BestDinProgramming {
        ...
        //O(N^2) - top down: mat[i][j] = mat[i][j-1] + a[j] // a[j] same as mat[j][j]
        public static int bestDP1(int[]a){
            ...
            for (int i=0; i<n; i++){
                for (int j=i+1; j<n; j++){
                    mat[i][j] = mat[i][j-1] + a[j]; // a[j] same as mat[j][j]
                    if (sumMax < mat[i][j]){
                        sumMax = mat[i][j];
                        beginMax = i;
                        endMax = j;
        ...
        //O(N^2) - bottom up: mat[i][j] = mat[i+1][j] + a[i]; a[i] same as mat[i][i]
        ...
        //O(N^2) mat[i][j] = mat[i][j-1] + mat[i-1][j] - mat[i-1][j-1];  another way
